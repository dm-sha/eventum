...
class EventumViewSet(EventumMixin, viewsets.ModelViewSet):
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        (либо публичная страница при public_page).
        """
    ...
    def filter_by_events(self, request, eventum_slug=None):
        ...
        if filter_type == 'participating':
            ...
            participants = Participant.objects.filter(
                id__in=participating_participant_ids,
                eventum=eventum
            )
            ...
            participants = Participant.objects.filter(
                eventum=eventum
            ).exclude(id__in=participating_participant_ids)
        
        # Оптимизируем запрос
        participants = participants.select_related('user', 'eventum')
...
class EventWaveViewSet(EventumScopedViewSet, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        # Создаем функцию для создания вложенного prefetch
        def create_nested_group_relations_prefetch(depth=3):
            """Создает prefetch для вложенных group_relations с указанной глубиной"""
            if depth <= 0:
                return []
        ...
        queryset = EventWave.objects.filter(eventum=eventum).select_related(
            'eventum'
        ).prefetch_related(
            'registrations',
            Prefetch(
                ...
                queryset=Event.objects.select_related(
                    ...
                    # Добавляем prefetch для registration и applicants (нужно для EventSerializer)
                    'registration',
...
def eventum_details(request, slug=None):
    ...
    try:
        ...
        # Участники и организаторы, либо публичная страница (public_page)
        from .auth_utils import get_user_role_in_eventum
        user_role = get_user_role_in_eventum(request.user, eventum) if request.user.is_authenticated else None
        if user_role not in ['organizer', 'participant'] and not eventum.public_page:
            return Response(
...
def participant_calendar_ics(request, eventum_slug=None, participant_id=None):
    ...
    try:
        ...
        # Получаем участника
        try:
            participant = Participant.objects.select_related('eventum').get(id=participant_id, eventum=eventum)
        except Participant.DoesNotExist:
